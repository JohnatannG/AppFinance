...
let Despesas =
  JSON.parse(localStorage.getItem(`FormDespesas_${userEmail}`)) || [];
let Receitas =
  JSON.parse(localStorage.getItem(`FormReceitas_${userEmail}`)) || [];
...
const saldoFormatado = registrationFormData.saldoUser.toLocaleString("pt-br", {
  style: "currency",
  currency: "BRL",
});
...
const modalSair = document
  .getElementById("modalSair")
  .addEventListener("click", function () {
    window.location.href =
      "https://app-finance-flame.vercel.app/pages/Login.html";
  });
const modalCancelar = document
...
const adicionarDespesas = document
  .getElementById("adicionarDespesas")
  .addEventListener("click", function () {
    const formAdicionarDespesas = document.getElementById(
      "formAdicionarDespesas"
    );
...
modalFormDespesas.addEventListener("submit", function (evento) {
  ...
  const totalDespesas = Despesas.reduce(
    (acc, despesa) => acc + despesa.valorDespesas,
    0
  );
  localStorage.setItem(
    `valorFormDespesas_${userEmail}`,
    JSON.stringify(totalDespesas)
  );
...
const adicionarReceitas = document
  .getElementById("adicionarReceitas")
  .addEventListener("click", function () {
    const formAdicionarReceitas = document.getElementById(
      "formAdicionarReceitas"
    );
...
const fecharModalReceitas = document
  .getElementById("fecharModalReceitas")
  .addEventListener("click", function () {
    const formAdicionarReceitas = document.getElementById(
      "formAdicionarReceitas"
    );
...
modalFormReceitas.addEventListener("submit", function (evento) {
  ...
  const totalReceitas = Receitas.reduce(
    (acc, receita) => acc + receita.valorReceitas,
    0
  );
  localStorage.setItem(
    `valorFormReceitas_${userEmail}`,
    JSON.stringify(totalReceitas)
  );
...
function mostrarSaldoReceitas() {
  const valorFormReceitas = JSON.parse(
    localStorage.getItem(`valorFormReceitas_${userEmail}`)
  );
  const totalReceitas = document.getElementById("totalReceitas");

  totalReceitas.textContent = `${valorFormReceitas.toLocaleString("pt-br", {
    style: "currency",
    currency: "BRL",
  })}`;
}
